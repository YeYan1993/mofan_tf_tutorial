...
def download():     # download tiger and kittycat image
    ...
    for category in categories:
        os.makedirs('./transfer_data/data/%s' % category, exist_ok=True)
        with open('./transfer_data/imagenet_%s.txt' % category, 'r') as file:
            urls = file.readlines()
            n_urls = len(urls)
            for i, url in enumerate(urls):
                try:
                    urlretrieve(url.strip(), './transfer_data/data/%s/%s' % (category, url.strip().split('/')[-1]))
